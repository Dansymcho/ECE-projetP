...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    m_benlever.set_bg_color(BLEUCLAIR);
    ...
    ///boite connexite
    m_top_box.add_child(m_bconnexe);
    m_bconnexe.set_dim(200,30);
    m_bconnexe.set_gravity_xy(grman::GravityX::Right, grman::GravityY::Up);
    m_bconnexe.set_bg_color(ROSE);

    ///message connexite
    m_bconnexe.add_child(m_connexe);
    m_connexe.set_message("Afficher infos connexes");
...
void Graph::update(int &u)
{
    ...
    if ( m_interface->m_bconnexe.clicked() )
    {

         std::cout<<"la matrice de forte connexite du graphe est:"<<std::endl;
         nbco(connexitetot());
    }
...
std::vector<int> Graph::connexite(int sommet)
{
    std::vector<std::vector<int>> matriceadj;
    std::vector<int> temp(m_vertices.size(),0);

    std::vector<std::vector<int>> Trmat;

    int nouv =1;
    int ordre = m_vertices.size();

    std::vector<int> c1(ordre,0);

    std::vector<int> c2(ordre,0);

    std::vector<int> c(ordre,0);

    std::vector<int> marques(ordre,0);

    for(int i=0; i<m_vertices.size(); i++)
    {
        matriceadj.push_back(temp);
        Trmat.push_back(temp);
    }
...
std::vector<std::vector<int>> Graph::connexitetot()
{
    int ordre=m_vertices.size();


    std::vector<std::vector<int>> tabc;
    std::vector<int>temp(ordre,0);
...
void Graph::nbco(std::vector<std::vector<int>> tabc)
{
    int ordre = m_vertices.size();
    int nb =0;
    int j=0;
    ...
    for(i=0; i<ordre; i++)
    {
        ...
        if(nb!=0)
        {
            std::cout<<"le sommmet :"<< i << " a "<< nb << "composantes connexes"<<std::endl;
