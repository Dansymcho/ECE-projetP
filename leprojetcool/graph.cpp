...
EdgeInterface::EdgeInterface(Vertex& from, Vertex& to)
{
    ...
    m_slider_weight.set_posy(20);

    // Label de visualisation de valeur
    m_box_edge.add_child( m_label_weight );
    m_label_weight.set_gravity_y(grman::GravityY::Down);

    /* m_box_edge.add_child( m_indice );
     m_indice.set_message();
     m_indice.set_gravity_y(grman::GravityY::Up);*/
...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    ///boite du menu2
    m_menu2.set_frame(800, 460, 200, 260);
    ...
    m_nouveau.set_message("Ajouter espèce");
    ...
    m_bretour.set_frame(12,200,180,30);
    ...
    ///boite ajouter arete
    m_menu2.add_child( m_bajoutera );
    m_bajoutera.set_frame(12,120,180,30);
    m_bajoutera.set_bg_color(BLEU);

    ///message arete
    m_bajoutera.add_child(m_ajoutera);
    m_ajoutera.set_message("Ajouter arete");

    ///boite enlever arete
    m_menu2.add_child( m_benlevera );
    m_benlevera.set_frame(12,160,180,30);
    m_benlevera.set_bg_color(BLEU);

    ///message enlever arete
    m_benlevera.add_child(m_enlevera);
    m_enlevera.set_message("Annuler arete");
...
void Graph::sauvegarde(std::string SfichS,std::string SfichA)
{
    ...
    {
        fich<<i; //id
        fich<<" "<<it.second.m_from; //S1
        fich<<" "<<it.second.m_to; //S2
        fich<<" "<<it.second.m_weight<<"\n"; //poid
        i++;
    }
...
void Graph::update(int &u)
{
    ...
    {
        std::cout << "Ajouter espèce" << std::endl;

        Ajoutespece(m_vertices);
    ...
    if ( m_interface->m_bajoutera.clicked() )
    {
        std::cout << "Ajouter arete" << std::endl;
        Ajoutarete(m_edges);
    }

    if ( m_interface->m_benlevera.clicked() )
    {
        std::cout << "Annuler arete" << std::endl;
...
void Graph::Ajoutespece(std::map<int, Vertex> m_vertices)
{
    ...
    add_interfaced_vertex(m_vertices.size(), 0.0,1.1, 650, 50, ajout);


}

void Graph::Ajoutarete(std::map<int, Edge> m_edges)
{
    int S1;
    int S2;
    int valeur;

    std::cout<<"Choisissez une espece de depart"<<std::endl;
    std::cin>> S1;
    std::cout<<"Choisissez une espece de d'arrive"<<std::endl;
    std::cin>> S2;
    std::cout<<"Choisissez une valeur"<<std::endl;
    std::cin>> valeur;
    std::cout<<"Arete ajoutee"<<std::endl;

    add_interfaced_edge(m_edges.size(),S1,S2,valeur);
}

void Graph::Enleversommet()
{
    int idxenlever;

    std::cout<<"Mettre l'indice du sommet à supprimer"<<std::endl;
    std::cin>>idxenlever;

    Vertex &remed = m_vertices.at(idxenlever);
    std::cout<<"Sommet supprimé:"<< idxenlever<<" "<<std::endl;

    if(m_interface && remed.m_interface)
    {
        m_interface->m_main_box.remove_child(remed.m_interface->m_top_box);
        m_vertices.erase(idxenlever);
    }
...
void Graph::caclul(int &jour)
{

        for(int j=0 ; j<m_vertices[i].m_in.size() ; j++) ///parcour de toutes les arretes d'un sommet
        {
            //std::cout<<m_vertices[m_edges[m_vertices[i].m_in[j]].m_to].m_value<<" ";
            //std::cout<<m_vertices[i].m_in.size()<<" ";
            std::cout<<j<<" ";
            /*std::string Cs = m_edges[m_vertices[i].m_in[j]].m_interface->m_label_weight.get_message();
            int Ci= std::stoi(Cs);

            std::string Ss = m_vertices[m_edges[m_vertices[i].m_in[j]].m_from].m_interface->m_label_value;
            int Si= std::stoi(Ss);

            K=K+ Ci*Si;*/
            int g=0;
            K=K+0,01*(m_edges[m_vertices[i].m_in[j]].m_weight * m_vertices[m_edges[m_vertices[i].m_in[j]].m_to].m_value); /// m_vertices[i].m_in[j] = indice des arretes / m_vertices[i].m_in[j]].m_from = sommet dont les dites aretes proviennent
            g=m_vertices[m_edges[m_vertices[i].m_in[j]].m_to].m_value;
            //std::cout<<g<<std::endl;

            //std::cout<<K<<" "<<std::endl;
            //Kbis=m_edges[m_vertices[i].m_in[j]].m_weight
        }
        for(int j=0; j<m_vertices[i].m_out.size(); j++)
        {
            L=L+0,01*(m_edges[m_vertices[i].m_out[j]].m_weight * m_vertices[m_edges[m_vertices[i].m_out[j]].m_to].m_value);
        ...
        if(K!=0)
        {
            ...
            std::cout<<"K est different de 0"<<std::endl;

            //std::cout<<K<<" ";
            float c =Ni+(Rf*Ni*(1-(Ni/K)))-L;
            m_vertices[i].m_value=c;
            std::string s = std::to_string(c);
            //std::cout<<s<<"  ";
            m_vertices[i].m_interface->m_box_label_crs.set_message(s);//set_value(N+R*N*(1-N/K)
        ...
        {
            m_vertices[i].m_interface->m_slider_value.set_value(1);
        }
...
/*void Graph::caclul(int &jour)
{
    for(int i =0; i<m_vertices.size(); i++) ///Parcour de tous les sommets
    {
        int o;
        o=m_vertices.size();
        //std::cout<<m_vertices.size()<< " ";
        float K(0);//coeficient prédateur
        float L(0); //coeficient de prédation


        //int N=m_vertices[i].m_value;
        std::string Ns=m_vertices[i].m_interface->m_label_value.get_message();
        int Ni=std::stoi(Ns);


        std::string Rs=m_vertices[i].m_interface->m_label_crs.get_message();
        float Rf=std::stof(Rs);
